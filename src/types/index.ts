...
export interface MoodleSiteInfo {
  sitename: string; username: string; firstname: string; lastname: string;
  fullname: string; lang: string; userid: number; siteurl: string; userpictureurl: string;
}
export interface AssignmentEvent {
  ...
  timesort: number;            // unix 秒 (締切)
  overdue: boolean;
  courseFullname: string;
  courseId: number;
  url: string;                 // moodle 上の課題 URL
  actionUrl?: string;          // 提出ページ
  modulename: string;          // 'assign' | 'quiz' など
}
